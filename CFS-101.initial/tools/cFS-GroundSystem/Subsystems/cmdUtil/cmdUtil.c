...
/*
** Process a word (32-bit) argument
*/
void ProcessWordArgument(char *optarg, CommandData_t *CommandData) {
    long      tempLong;
    int32_t   tempInt;

    tempLong = strtol(optarg, NULL, 0);
    if (tempLong > 0xFFFF) {
        fprintf(stderr, "Word Argument: '%s' rejected. Number is too large.\n", optarg);
        return;
    }

    tempInt = (int32_t)tempLong;
    if (hostByteOrder != CommandData->Endian) {
        byteSwap((char*)&tempInt, sizeof(tempInt));
    }

    /* Pack the data as comes, which may not be aligned. */
    memcpy(&CommandData->PacketData[CommandData->DataLen], &tempInt, sizeof(tempInt));
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    while( opt != -1 )
    {
        ...
        case 'w':
            ProcessWordArgument(optarg, &CommandData);
            break;
