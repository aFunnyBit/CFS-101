...
void CI_TaskInit(void)
{
    ...
    {
        CFE_EVS_SendEvent(CI_SOCKETCREATE_ERR_EID,CFE_EVS_ERROR,"CI_LAB: create socket failed = %d", errno);
    }
    else
    {
       ...
       {
           CFE_EVS_SendEvent(CI_SOCKETBIND_ERR_EID,CFE_EVS_ERROR,"CI_LAB: bind socket failed = %d", errno);
       }
       else
       {
...
MsgId = CFE_SB_GetMsgId(CIMsgPtr);
    ...
    switch (MsgId)
    {
        ...
        default:
			...
			"CI_LAB: invalid command packet,MID = 0x%x", MsgId);
            break;
    }
...
CI_DropPDUCmd_t *CmdPtr;
  ...
  */
  if (CI_VerifyCmdLength(msg, ExpectedLength))
  {
    ...
    CI_HkTelemetryPkt.ci_command_count++;
    CFE_EVS_SendEvent(CI_DROP_PDU_CMD_EID, CFE_EVS_DEBUG, "CI_LAB: Drop PDU\n");
  }
  ...
  if (CI_VerifyCmdLength(msg, ExpectedLength))
  {
    ...
    if (CmdPtr->PDUMsgID <= CFE_SB_HIGHEST_VALID_MSGID)
    {
      ...
      CFE_EVS_SendEvent(CI_CAPTUREPDU_CMD_EID, CFE_EVS_DEBUG,
                "CI_LAB: PDU Capture initialized for 0x%04X\n",CmdPtr->PDUMsgID);
    }
    else
    {
      CI_HkTelemetryPkt.ci_command_error_count++;
      CFE_EVS_SendEvent(CI_INVALID_MSGID_ERR_EID, CFE_EVS_ERROR,
              "CI_LAB: Invalid PDU MsgID: 0x%04x\n",CmdPtr->PDUMsgID);
    }
  }
  ...
  if (CI_VerifyCmdLength(msg, ExpectedLength))
  {
    if (PDUMessageID != 0)
    {
      ...
      CFE_EVS_SendEvent(CI_STOP_PDUCAPTURE_CMD_EID, CFE_EVS_DEBUG,
                "CI_LAB: PDU Capture stopped for 0x%04X\n",PDUMessageID);
    ...
    else
    {
      ...
      CFE_EVS_SendEvent(CI_NOCAPTURE_ERR_EID, CFE_EVS_ERROR,
              "CI_LAB: PDU Capture is not enabled\n");
    }
  }
...
/*    OS_printf("CI_LAB: Captured PDU with length = %d\n",PduHdrPtr->PDataLen); */
    ...
    {
      OS_printf("CI_LAB: File Data PDU rcv'd\n");
      if ((dropFileData == TRUE) && (dropFileDataCnt > 0))
      {
	dropFileDataCnt--;
        OS_printf("CI_LAB: File Data PDU dropped\n");
    ...
    else
    {
      ...
      OS_printf("CI_LAB: PDU Data Type = %d\n",PduData0);
      switch (PduData0)
      {
	case 4:
          OS_printf("CI_LAB: EOF PDU rcv'd\n");
          if ((dropEOF == TRUE) && (dropEOFCnt > 0))
	  {
	    dropEOFCnt--;
            OS_printf("CI_LAB: EOF PDU dropped\n");
	  ...
	  {
            OS_printf("CI_LAB: good checksum = %x\n",(unsigned int)*checkSumPtr);
	    /* Corrupt the checksum */
	    *checkSumPtr = 0x12345678;
            OS_printf("CI_LAB: corrupted checksum = %x\n",(unsigned int)*checkSumPtr);
	    corruptChecksum = FALSE;
	  }

          if (adjustFileSize == TRUE)
	  {
            OS_printf("CI_LAB: good file size = %d\n",(int)*fileSizePtr);
	    /* Adjust the file size */
	    *fileSizePtr += PDUFileSizeAdjustment;
            OS_printf("CI_LAB: adjusted file size = %d\n",(int)*fileSizePtr);
	    adjustFileSize = FALSE;
	  }
	...
	case 5:
          OS_printf("CI_LAB: FIN PDU rcv'd\n");
          if ((dropFIN == TRUE) && (dropFINCnt > 0))
	  {
	    dropFINCnt--;
            OS_printf("CI_LAB: FIN PDU dropped\n");
	...
	case 6:
          OS_printf("CI_LAB: ACK PDU rcv'd\n");
          if ((dropACK == TRUE) && (dropACKCnt > 0))
	  {
	    dropACKCnt--;
            OS_printf("CI_LAB: ACK PDU dropped\n");
	...
	case 7:
          OS_printf("CI_LAB: Meta Data PDU rcv'd\n");
          if ((dropMetaData == TRUE) && (dropMetaDataCnt > 0))
	  {
	    dropMetaDataCnt--;
            OS_printf("CI_LAB: Meta Data PDU dropped\n");
	...
	case 8:
          OS_printf("CI_LAB: NAK PDU rcv'd\n");
          if ((dropNAK == TRUE) && (dropNAKCnt > 0))
	  {
	    dropNAKCnt--;
            OS_printf("CI_LAB: NAK PDU dropped\n");
