...
void TO_StartSending( TO_OUTPUT_ENABLE_PKT_t * pCmd )
{
    ...
    CFE_EVS_SendEvent(TO_TLMOUTENA_INF_EID,CFE_EVS_INFORMATION,"TO_LAB: Telemetry output enabled for IP %s", tlm_dest_IP);
       ...
       switch (CFE_SB_RcvMsg(&MsgPtr, TO_Cmd_pipe, CFE_SB_POLL))
       {
          case CFE_SUCCESS:
               ...
               switch (MsgId)
               {
                  ...
                  default:
                       CFE_EVS_SendEvent(TO_MSGID_ERR_EID,CFE_EVS_ERROR, "L%d TO_LAB: Invalid Msg ID Rcvd 0x%x",__LINE__,MsgId);
                       break;
               }
               break;
            default:
...
void TO_exec_local_command(CFE_SB_MsgPtr_t cmd)
{
    ...
    {
       case TO_NOP_CC:
            CFE_EVS_SendEvent(TO_NOOP_INF_EID,CFE_EVS_INFORMATION, "TO_LAB: NOOP command");
            break;
       ...
       default:
            CFE_EVS_SendEvent(TO_FNCODE_ERR_EID,CFE_EVS_ERROR, "L%d TO_LAB: Invalid Function Code Rcvd In Ground Command 0x%x",__LINE__,
                              CommandCode);
            valid = FALSE;
    }
...
void TO_reset_status(void)
{
    ...
    CFE_EVS_SendEvent(TO_NOOP_INF_EID,CFE_EVS_INFORMATION, "TO_LAB: RESET command");
